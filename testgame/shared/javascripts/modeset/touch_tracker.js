...
// prevent clicking/dragging on children from interfering with container's dragging
MouseAndTouchTracker.recurseDisableElements = function ( elem, disabledElements ) {
  if( elem ) {
    // disable clicking/dragging on selected element types
    ...
    if( elem.childNodes.length > 0 ){
      for( var i=0; i < elem.childNodes.length; i++ ) {
        MouseAndTouchTracker.recurseDisableElements( elem.childNodes[i], disabledElements );
      }
    }
  }
};

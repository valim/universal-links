...
Accelerometer.HAS_ORIENTATION = false;
...
Accelerometer.watch = function( callback ) {
  ...
  if( Accelerometer.ACTIVE == false ) {
    if( window.addEventListener ) {
      ...
      window.addEventListener('devicemotion', Accelerometer.updateMotion, false);
      window.addEventListener('deviceorientation', Accelerometer.updateOrientation, false);
    }
  }
};
...
Accelerometer.unwatch = function( callback ) {
  ...
  if( Accelerometer.callback.length === 0 ) {
    if( window.removeEventListener ) {
      Accelerometer.ACTIVE = false;
      window.removeEventListener('devicemotion', Accelerometer.updateMotion);
      window.removeEventListener('deviceorientation', Accelerometer.updateOrientation);
    }
  }
};

Accelerometer.updateMotion = function( event ) {
  if( !event ) return;
...
Accelerometer.updateOrientation = function( event ) {
  if( !event ) return;

  // set available states
  Accelerometer.AVAILABLE = true;
  if( event.gamma != undefined || event.beta != undefined || event.alpha != undefined ) Accelerometer.HAS_ORIENTATION = true;

    // tilt - horiz = 0 when flat on a table, vert = 0 when standing straight up
  if( Accelerometer.HAS_ORIENTATION ) {
    if( MobileUtil.isPortrait ) {
        Accelerometer.tiltZDegrees = event.gamma;
        Accelerometer.tiltXDegrees = event.beta;
        Accelerometer.compassDegrees = event.alpha;
    } else {
        // Accelerometer.tiltXHoriz = ( MobileUtil.orientation == MobileUtil.LANDSCAPE_LEFT ) ? event.accelerationIncludingGravity.x : -event.accelerationIncludingGravity.x;
        // Accelerometer.tiltZ = ( MobileUtil.orientation == MobileUtil.LANDSCAPE_RIGHT ) ? event.accelerationIncludingGravity.y : -event.accelerationIncludingGravity.y;
        // Accelerometer.tiltXVert = event.accelerationIncludingGravity.z;
...
Accelerometer.empty = function() {};
Accelerometer.detectAccelerometer = function( callback ) {
  Accelerometer.watch( Accelerometer.empty ) 
  setTimeout(function(){
    if( Accelerometer.HAS_ORIENTATION == true || Accelerometer.HAS_ACCEL == true ) {
      callback(true);
      Accelerometer.unwatch( Accelerometer.empty );
    } else {
      callback(false);
    }
  },1000);
    //   Accelerometer.unwatch()
    //   # check for the cross-platform 'deviceorientation' event
    //   @hasAccelerometer = Accelerometer.HAS_ORIENTATION 
    //   @finishPrerequisiteCheck()
    //   # alert 'accel: '+Accelerometer.HAS_ORIENTATION+', '+Accelerometer.HAS_ACCEL+', '+Accelerometer.HAS_GYRO
    // , 1500
};

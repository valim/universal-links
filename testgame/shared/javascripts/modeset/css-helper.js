...
var degreesToRadians = function( d ) {
  return d * ( Math.PI / 180 );
    ...
    var update2DPosition = function ( element, x, y, scale, rot, keepTransition, raphObj ) {
        ...
        if( !_transformsEnabled ) {
            ...
            // if( raphObj ) {
            //     raphObj.animate({ transform: ["R",rot,0,0] });
            // }

            // sort-of a fix for IE, but really not...
            // if( rot != 0 ) {
            //     element.style.filter = 'progid:DXImageTransform.Microsoft.Matrix(M11='+ Math.cos(degreesToRadians(rot)) +', M12='+ -Math.sin(degreesToRadians(rot)) +', M21='+ Math.sin(degreesToRadians(rot)) +', M22='+ Math.cos(degreesToRadians(rot)) +', SizingMethod="auto expand")';
            // }
...
CSSHelper.getVendorPrefix = function( styleSuffix ) {
    ...
    var checkVendor = function() {
        if(!navigator.userAgent.toLowerCase().match(/msie 9/i)){
            var vendors = ['Moz', 'Webkit', 'O','ms'];   // should have 'ms' also, but IE9 transform doesn't work, even though it claims to exist. so, we leave it out
